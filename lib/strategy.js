...
function Strategy(options, verify) {
  ...
  this._verify = verify;
  this._oauth2 = this._getOAuth2Client(options);
  ...
  this._claimsInToken = !!options.claimsInToken;

  if (this._claimsInToken) {
  ...
  if (req.query && req.query.code) {
    ...
    function loaded(err, ok, state) {
      ...
      if (self._oauth2) {
          ...
          oauth2 = self._oauth2 = self._getOAuth2Client(meta);
      }
      ...
      oauth2.getOAuthAccessToken(code, { grant_type: 'authorization_code', redirect_uri: callbackURL }, function(err, accessToken, refreshToken, params) {
        ...
        //   console.log('XXXXXX', err, accessToken, refreshToken, params);
        ...
        var idTokenSegments = idToken.split('.'),
            accessTokenSegments = accessToken.split('.'),
            accessTokenStr,
            accessTokenClaims,
            jwtClaimsStr,
            jwtClaims;
        ...
        try {
            ...
            accessTokenStr = new Buffer(accessTokenSegments[1], 'base64').toString();
            jwtClaims = JSON.parse(jwtClaimsStr);
            accessTokenClaims = JSON.parse(accessTokenStr);
        } catch (ex) {
          return self.error(ex);
        }

        // console.log('@@@@@', jwtClaims, accessTokenClaims);
        // console.log('-----');

        var missing = ['iss', 'sub', 'aud', 'exp', 'iat'].filter(function (param) {
        ...
        if (jwtClaims.iss !== meta.issuer) {
            return self.error(
                new Error('id token not issued by correct OpenID provider - ' +
        ...
        if (typeof jwtClaims.aud === 'string') {
            if (jwtClaims.aud !== meta.clientID) {
                return self.error(
                    new Error('aud parameter does not include this client - is: ' +
        ...
        self._shouldLoadUserProfile(iss, sub, function(err, load) {
          ...
          if (load) {
            ...
            oauth2._request("GET", userInfoURL, { 'Authorization': "Bearer " + accessToken, 'Accept': "application/json" }, null, null, function (err, body, res) {
                ...
                // console.log('###', userInfoURL, 'Authorization: ' + "Bearer " + accessToken);

                // console.log('@@@@@@@@@', userInfoURL, err, body, res);
              ...
              try {
                ...
                json.token = {
                    id_token: idToken, // send idToken back
                    access_token: accessToken, // send accessToken back
                    refresh_token: refreshToken // send refreshToken back
                };
              ...
              if (self._claimsInToken) {
                  var name = [];

                  name.push(accessTokenClaims.given_name);
                  if (accessTokenClaims.middle_name) {
                      name.push(accessTokenClaims.middle_name);
                  }
                  name.push(accessTokenClaims.family_name);

                  var profile = {
                      id: sub,
                      displayName: name.join(' '),
                      name: {
                          familyName: accessTokenClaims.family_name,
                          givenName: accessTokenClaims.given_name,
                          middleName: accessTokenClaims.middle_name
                      },
                      _json: accessTokenClaims
                  };
                ...
                //   console.log('PROFILE', profile);
                  ...
                  return onProfileLoaded(profile);
    ...
    this._setup(identifier, function(err, config) {
      ...
      if (config.acrValues) {
          params['acr_values'] = config.acrValues;
      }
